...
def stand_errors(demy1_fn, demy2_fn, dh_fn, outlines, statsfn, missing):
    ...
    df.to_csv(statsfn+'df_stats.csv')
    ...
    with open(statsfn+'dh_err.pickle', 'wb') as handle:
    ...
    df_vgm.to_csv(statsfn+'variogram.csv')
    print('variogram')
    df_vgm = pd.read_csv(statsfn+'variogram.csv')
    # 
    ...
    df2 = pd.DataFrame(columns=['slope', 'maxc', 'z_error', 'nrsamples', 'dh', 'dh_err'], index=glacier_outlines.ds["nr"].values)
    ...
    for gl in glacier_outlines.ds["nr"]:
        ...
        # if gl not in [2167, 2168, 2169, 2170, 2171]:
            ...
            df2.loc[gl, 'dh'] = gl_dh
            df2.loc[gl, 'dh_err'] = gl_dh_err
        df2.to_csv(statsfn+'error_dataframe.csv')
    ...
    plt.savefig(statsfn+'variogram.png')
    # plt.show()
