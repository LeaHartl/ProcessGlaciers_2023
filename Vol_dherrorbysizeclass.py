...
def makeVariogramComputeErrors(demy1_fn, demy2_fn, dh_fn, outlines, statsfn, sizeclasses):
    ...
    df.to_csv('outputxdem/'+statsfn+'df_stats2.csv')
    ...
    df_vgm.to_csv('outputxdem/'+statsfn+'variogram2.csv')
    #     # %%
    ...
    df_vgm = pd.read_csv('outputxdem/'+statsfn+'variogram2.csv')
    ...
    df2 = pd.DataFrame(columns=['slope', 'maxc', 'z_error', 'nrsamples', 'dh', 'dh_err'],index=['all', 'vsmall', 'small', 'mid', 'big', 'vbig'])
    clnms = ['all', 'vsmall', 'small', 'mid', 'big', 'vbig']
    for i, cl in enumerate(sizeclasses):
        clnm = clnms[i]
        print(clnm)
    #     if missing == 'no':
        # if gl not in [2167, 2168, 2169, 2170, 2171]:
            # print(gl)
        gl_shp_cl = gu.Vector(cl)
        gl_mask = gl_shp_cl.create_mask(dh)

        nmeanslope = np.nanmean(slope[gl_mask])
        nmeanmaxc = np.nanmean(maxc[gl_mask])

        df2.loc[clnm, 'slope'] = nmeanslope
        df2.loc[clnm, 'maxc'] = nmeanmaxc

        gl_neff = xdem.spatialstats.neff_circular_approx_numerical(
            area=gl_shp_cl.ds.area.values[0], params_variogram_model=params_vgm
        )

        df2.loc[clnm, 'nrsamples'] = gl_neff

        gl_z_err = 1 / np.sqrt(gl_neff)


        df2.loc[clnm, 'z_error'] = gl_z_err
        ...
        df2.loc[clnm, 'dh'] = gl_dh
        df2.loc[clnm, 'dh_err'] = gl_dh_err
    df2.to_csv('outputxdem/'+statsfn+'error_dataframe_classes.csv')
    ...
    plt.savefig('outputxdem/'+statsfn+'variogram_all_new.png')
    # plt.show()
...
classesGI2 = makeSizeclasses(GI2)
classesGI3 = makeSizeclasses(GI3)

# makeVariogram(fn2006_5al, fn2017_5al, dh_2006, GI1, '20062017_', 'yes')


# makeVariogramComputeErrors(fn2006_5al, fn2017_5al, dh_2006, GI1, '20062017_', classesGI3)
# makeVariogramComputeErrors(fn2006_5al, fn2017_5al, dh_2006_ma, GI1, '20062017_ma', classesGI3)

makeVariogramComputeErrors(fn1997_5al, fn2006_5al, dh_1997_ma, GI1, '19972006_ma', classesGI2)
